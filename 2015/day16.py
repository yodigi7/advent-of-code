...
def load_sues() -> list[Sue]:
    ...
    for line in input.splitlines():
        ...
        if "children:" in words:
            children = int(words[words.index("children:") + 1].strip(","))
        if "cats:" in words:
            cats = int(words[words.index("cats:") + 1].strip(","))
        if "samoyeds:" in words:
            samoyeds = int(words[words.index("samoyeds:") + 1].strip(","))
        if "pomeranians:" in words:
            pomeranians = int(words[words.index("pomeranians:") + 1].strip(","))
        if "akitas:" in words:
            akitas = int(words[words.index("akitas:") + 1].strip(","))
        if "vizslas:" in words:
            vizslas = int(words[words.index("vizslas:") + 1].strip(","))
        if "goldfish:" in words:
            goldfish = int(words[words.index("goldfish:") + 1].strip(","))
        if "trees:" in words:
            trees = int(words[words.index("trees:") + 1].strip(","))
        if "cars:" in words:
            cars = int(words[words.index("cars:") + 1].strip(","))
        if "perfumes:" in words:
            perfumes = int(words[words.index("perfumes:") + 1].strip(","))
        sues.append(
            Sue(
                num,
                children,
...
if __name__ == "__main__":
    ...
    for sue in sues:
        if (
            (sue.children != None and sue.children != 3)
            or (sue.cats != None and sue.cats != 7)
            or (sue.samoyeds != None and sue.samoyeds != 2)
            or (sue.pomeranians != None and sue.pomeranians != 3)
            or (sue.akitas != None and sue.akitas != 0)
            or (sue.vizslas != None and sue.vizslas != 0)
            or (sue.goldfish != None and sue.goldfish != 5)
            or (sue.trees != None and sue.trees != 3)
            or (sue.cars != None and sue.cars != 2)
            or (sue.perfumes != None and sue.perfumes != 1)
        ):
            continue
        print(sue.number)
        break
