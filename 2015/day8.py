...
def calc_chars(string):
    string = string[1:-1]
    string = string.replace("\\\\", "x").replace('\\"', "x")
    string = sub(r"\\x..", "x", string)
    return len(string)


def calc_chars_part_2(string):
    string = string.replace("\\", "\\\\").replace('"', '\\"')
    # string = sub("x", r"\\x..", string)
    return len(f'"{string}"')


if __name__ == "__main__":
    print(
        f"Difference: {sum(len(line) - calc_chars(line) for line in input.splitlines())}"
    )
    print(
        f"Difference 2: {sum(calc_chars_part_2(line) - len(line) for line in input.splitlines())}"
    )
