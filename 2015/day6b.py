...
def read_instruction(string):
    ...
    if split_string[0] == "toggle":
        return split_string[0], read_box(split_string[1]), read_box(split_string[3])
    else:
        return (
            " ".join((split_string[0], split_string[1])),
            read_box(split_string[2]),
            read_box(split_string[4]),
        )


def yield_coords(box1, box2):
    for y in range(box1[1], box2[1] + 1):
        for x in range(box1[0], box2[0] + 1):
            yield x, y


def turn_on(box1, box2):
    for x, y in yield_coords(box1, box2):
        board[x][y] += 1


def turn_off(box1, box2):
    for x, y in yield_coords(box1, box2):
        board[x][y] -= 1
        board[x][y] = max(board[x][y], 0)


def toggle(box1, box2):
    for x, y in yield_coords(box1, box2):
        board[x][y] += 2


command_to_fn = {"toggle": toggle, "turn off": turn_off, "turn on": turn_on}
board = [[0 for _ in range(1000)] for _ in range(1000)]

if __name__ == "__main__":
    for instruction in input.splitlines():
        command, box1, box2 = read_instruction(instruction)
        command_to_fn[command](box1, box2)

    print(f"Brightness: {sum(sum(board, []))}")
